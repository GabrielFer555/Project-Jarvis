# Arquitetura — Chat texto e debug

## Contexto

O cérebro já aceita texto (`generate_reply`). O gargalo para testar a LLM é o canal: hoje só entra áudio.
...
O modo texto é um segundo ponto de entrada, no pacote `agent`. Não substitui a voz. O debug no Cursor lança um desses dois processos com o debugger anexado.
...
| Testes | `tests/test_text_chat.py` (e roteamento) | Flags, vazio, sair, erro, idioma |
...
| Import preguiçoso de `voice` em `main.py` | `voice` só entra no ramo sem `--text`; o modo texto não carrega microfone, Whisper, Piper nem a wake word. A Etapa 2 original não dizia isso explicitamente; decisão da Fase 1. |
