...
- `test-results/` está no `.gitignore`: XML gerado localmente não é versionado.

## Impacto nas regras existentes

| Doc afetada | Regra vigente | Passa a valer |
| --- | --- | --- |
| `docs/padroes-de-implementacao.md` | Comandos locais `py -3.11`, `compileall`, `unittest`; pytest fora da stack | O mesmo build/teste passa a rodar também no GitHub Actions (PRs para `main`/`develop`): fim da exclusividade local. XML só no runner (`unittest-xml-reporting`). pytest continua fora |
| *(demais `docs/<funcionalidade>/`)* | Sem CI | Sem mudança de regra de produto |

Nenhuma regra de voz, cérebro, memória, API, rosto, chat ou spec muda. Funcionalidade nova: `docs/ci/`.
